...
- `assets/image/main_reference.png`：游戏主界面参考图资源，运行时路径 `image/main_reference.png`。
- `.project/`：构建后由工具生成。
- `docs/`：记忆系统目录。
...
- 创建参考图风格的游戏主界面，使用 `image/main_reference.png` 作为临时整屏背景参考，并叠加资源栏、顶部菜单、章节条、模式入口、领取栏、功能按钮、底部导航等 Yoga UI。
- LSP 诊断 0 Error，官方构建工具构建成功。
