...
- `scripts/Formation/FormationScene.lua`：独立勇者 NPC 上阵编队页面，包含三套阵容 TAB、实时总战力、勇者列表排序、6 格前后排站位、上阵/替换/下阵、单格锁定、全阵容锁定、一键上阵、一键清空、保存阵容、推荐开关与羁绊展示。
...
- 新增独立勇者 NPC 上阵编队页面：`FormationScene` 支持三套阵容 TAB、实时总战力、勇者列表按战力/品质/职业排序、6 格前后排站位、上阵/替换/下阵、单格锁定、全阵容锁定、一键上阵、一键清空、保存阵容、推荐开关与羁绊展示；主界面底部“阵型”入口已接入；`SaveSchema` 默认提供 6 名勇者和 3 套独立阵容存档，并在编队修改时通过 `SaveManager.SaveGameSnapshot()` 更新存档。
