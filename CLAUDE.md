...
- 勇者动作资源位于 `assets/image/npcClip/0001/` 等 NPC 序列帧目录，主界面森林关卡区 `Hanginglist` 最多显示 5 个已拥有勇者，拥有不足 5 个时只显示实际拥有数量；显示勇者会按状态播放待机 01-04、移动 06-09、攻击 10-14 序列帧，三类动作已统一显示高度，攻击帧使用更宽承载框避免人物变小；主界面、编队页、战斗页和 `NormalizedSprite` 已对缺失 NPC 序列帧做兜底，若 `image/npcClip/0171/01.png` 等资源不存在会回退到 `image/npcClip/0001/`，存在性检查使用 `cache:Exists()` 避免资源缺失 ERROR；点击主界面 `Hanginglist` 中的勇者或“勇者”圆形入口会进入勇者养成页，勇者养成页聚焦升级、升星和属性展示，右侧详情会显示勇者概览、具体属性（生命/攻击/防御/移动）、升级和升星；学习技能与技能升级在独立 `HeroSkillScene` 中完成，由主界面底部“技能”导航进入，返回时回主界面，技能等级保存在 `hero.skillLevels`；左侧勇者列表只在页面打开时重置，选择勇者和养成操作只局部刷新右侧详情区并更新左侧卡片选中态/等级/战力/星级文本，避免列表焦点丢失。
...
- 最近一次功能修改已将技能培养入口迁移到主界面：`HeroGrowthScene` 移除“技能培养”面板、`onOpenSkill` 回调和相关技能入口文案，勇者养成页只保留升级、升星和属性展示；主界面底部第 3 个导航改为“技能”，点击进入 `HeroSkillScene`，技能页关闭后返回主界面。
