...
- 游戏主界面已从整屏参考图切换为分层资源：背景、森林关卡、资源图标、顶部功能图标、挑战徽章、收益条、秘境按钮、功能按钮、底部导航均使用 `assets/image/` 下的独立图片；顶部 HUD 的 `time` 按 24 小时制显示当前时间，`coinValue`/`diamondValue`/`crystalValue` 绑定玩家金币、蓝钻、白钻并在数据变化或页面刷新时同步更新；主界面顶部“设置”按钮临时作为清档入口，会删除当前云存档所有 key 并返回登录界面重新进游戏；主界面“秘境挑战”按钮会打开与背包页顶部/底部背景一致的秘境挑战界面：根节点使用 `image/page_background.png`，底部返回按钮使用 `image/BT-返回.png`，左侧展示当前章节关卡和首通掉落，右侧展示选中关卡阵型、总战力、首通奖励或已通关扫荡奖励。
